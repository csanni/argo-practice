...
Write-Host "Checking for prerequisites (Podman mode)..."
if (-not (Get-Command "podman" -ErrorAction SilentlyContinue)) {
    Write-Error "Podman is not installed. Please install Podman."
    exit 1
}
if (-not (Get-Command "kind" -ErrorAction SilentlyContinue)) {
    Write-Error "Kind is not installed. Please install Kind."
    exit 1
}
...
if (kind get clusters | Select-String -Pattern "^$ClusterName$") {
    ...
    Write-Host "Creating Kind cluster '$ClusterName' with Podman..."
